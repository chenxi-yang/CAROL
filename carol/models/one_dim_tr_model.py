...
import carol.models.util as model_util
...
class OneDTransitionRewardModel(Model):
    ...
    To use with :class:carol.models.ModelEnv`, the wrapped model must define methods
    ``reset_1d`` and ``sample_1d``.

    Args:
        model (:class:`carol.model.Model`): the model to wrap.
        ...
        obs_process_fn (callable, optional): if provided, observations will be passed through
            this function before being given to the model (and bef ore the normalizer also).
    ...
    def __init__(
        ...
        obs_process_fn: Optional[carol.util.types.ObsProcessFnType] = None,
        ...
        self.model = model
        self.input_normalizer: Optional[carol.util.math.Normalizer] = None
        if normalize: # true in core
            self.input_normalizer = carol.util.math.Normalizer(
                self.model.in_size,
                self.model.device,
        ...
        if model_normalization is not None:
            self.model_normalization = model_normalization

    def _get_model_input(
        self,
        obs: carol.util.types.TensorType,
        action: carol.util.types.TensorType,
    ) -> torch.Tensor:
        ...
        if isinstance(obs, torch.Tensor):
            ...
            model_in = obs.concatenate(action)
    ...
    def _process_batch(
        self, batch: carol.util.types.TransitionBatch, _as_float: bool = False
    ...
    def update_normalizer(self, batch: carol.util.types.TransitionBatch):
        ...
        Args:
            batch (:class:`carol.util.types.TransitionBatch`): The batch of transition data.
    ...
    def loss(
        self,
        batch: carol.util.types.TransitionBatch,
    ...
    def get_output_and_targets(
        self, batch: carol.util.types.TransitionBatch
    ...
    def sample(
        ...
        """
        # if model_state, act are symbolic, obs, model_in are symbolic as well
        ...
        # print(f"in Sample: model_in: {model_in}; model_state: {model_state}")
        # print(f"deterministic: {deterministic}")
        preds, next_model_state = self.model.sample_1d(
            model_in, model_state, rng=rng, deterministic=deterministic
        )
        # print(f"## Sample: preds: {preds}; next_model_state: {next_model_state}")
        # print(f"preds: {preds.shape}")
        # print(f"next model_state: {next_model_state['obs'].shape}")
        # preds = preds[0] # for gaussian_mlp and truncated_normal_mlp

        if isinstance(preds, torch.Tensor):
            next_observs = preds[:, :-1] if self.learned_rewards else preds
        else: 
            obs_dimension = obs.shape[1]
            next_observs = preds.select_from_index(1, [_ for _ in range(obs_dimension)]) if self.learned_rewards else preds
        if self.target_is_delta:
            ...
            next_observs = tmp_
        # print(f"target_is_delta: {self.target_is_delta}")
        # print(f"## Sample, next_observs: {next_observs}")
        if isinstance(preds, torch.Tensor):
            rewards = preds[:, -1:] if self.learned_rewards else None
        else:
            rewards = preds.select_from_index(1, [obs_dimension]) if self.learned_rewards else None
            if not symbolic:
                rewards = rewards.inf() # select the inf of the symbolic rewards
    ...
    def reset(
        ...
        obs = model_util.to_tensor(obs).to(self.device) # this covers the Box type
        model_state = {"obs": obs}
        model_state.update(self.model.reset_1d(obs, rng=rng))
        return model_state

    def save(self, save_dir: Union[str, pathlib.Path]):
        # cx: direct save to the work_dir
        # cx: if exists, overlaps it
