...
class GaussianMLP(Ensemble):
    ...
    def forward(  # type: ignore
        ...
        if use_propagation:
            ...
            # )
            mean, logvar = self._default_forward(x, only_elite=False)
            # print(f"self.num_members: {self.num_members}")
            if self.num_members == 1:
                mean = mean[0]
                logvar = logvar[0] if logvar is not None else None
    ...
    def _nll_loss(self, model_in: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        nll = (
            carol.util.math.gaussian_nll(pred_mean, pred_logvar, target, reduce=False)
    ...
    def eval_score(  # type: ignore
        ...
        with torch.no_grad():
            ...
            # pred_mean = self.forward(model_in, use_propagation=False)
            target = target.repeat((self.num_members, 1, 1))
            # print(f"pred_mean: {pred_mean.shape}, target: {target.shape}")
