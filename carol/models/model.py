...
from carol.models.util import to_tensor
from carol.util.types import ModelInput, TransitionBatch
...
class Ensemble(Model, abc.ABC):
    ...
    def sample_1d(
        ...
        '''
        means: represented as box
        logvars: represented concretely
        '''
        # print(f"means, logvars: {means.shape, logvars.shape}")
        variances = logvars.exp()
        stds = torch.sqrt(variances)
        # return torch.normal(means, stds, generator=rng), model_state
        
        # preds = means.add(torch.randn(means.shape).to(means.device) * stds)

        # preds = means# + N(0, 1)*stds
        # The same as below lines (2).
        # Wrong: eps = torch.normal(torch.zeros(means.shape).to(means.device), stds, generator=rng)
        # Correct: eps = torch.normal(torch.zeros(means.shape).to(means.device), torch.ones(stds).., generator=rng)
        std_ones = torch.ones(stds.shape).to(stds.device)
        eps = torch.normal(torch.zeros(means.shape).to(means.device), std_ones, generator=rng)
        # preds = means.add(eps * stds)
        preds = means.add(eps * stds)
