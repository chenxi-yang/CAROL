...
class ModelEnv:
    ...
    Args:
        ...
        model (:class:`carol.models.Model`): the model to wrap.
    ...
    def __init__(
        ...
        env: gym.Env,
        model: Model,
        termination_fn: carol.util.types.TermFnType,
        reward_fn: Optional[carol.util.types.RewardFnType] = None,
    ...
    def reset(
        ...
        """
        if isinstance(self.dynamics_model, carol.models.OneDTransitionRewardModel):
    ...
    def step(
        self,
        actions: carol.util.types.TensorType,
        model_state: Dict[str, torch.Tensor],
        sample: bool = False,
        symbolic: bool = False,
    ) -> Tuple[carol.util.types.TensorType, carol.util.types.TensorType, np.ndarray, Dict]:
        # in Rollout, sample is True
        ...
        """
        # print(actions.shape)
        if len(actions.shape) == 1: # add the batch dim
        ...
        with torch.no_grad():
            ...
            if isinstance(actions, np.ndarray):
                ...
                # Can I omit this part to a deterministic model?
            # print(f"in step: \n model_state: {model_state}, actions: {actions}")
            (
                next_observs,
                pred_rewards,
                pred_terminals,
            ...
            rewards = (
                pred_rewards
                if self.reward_fn is None #  always None
                else self.reward_fn(actions, next_observs)
            )
            # TODO: symbolic version, mask batch: (for now, if all the states can give done, then 1; otherwise, 0)
            if isinstance(actions, torch.Tensor):
                dones = self.termination_fn(actions, next_observs)
            else: # for the symbolic version, update the mask batch by the center. Todo: use the lower bound of dones
                dones = self.termination_fn(actions.c, next_observs.c)
            ...
            if symbolic: # return the cuda reward
