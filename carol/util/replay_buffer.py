...
from .types import TransitionBatch
...
class SymbolicReplayBuffer:
    """A symbolic replay buffer with support for training/validation iterators and ensembles.
    ...
    Args:
        ...
        obs_shape (Sequence of ints): the shape of the observations to store.
        action_shape (Sequence of ints): the shape of the actions to store.
        obs_type (type): the data type of the observations (defaults to box<np.float32>).
        action_type (type): the data type of the actions (defaults to box<np.float32>).
        reward_type (type): the data type of the rewards (defaults to np.float32).
        rng (np.random.Generator, optional): a random number generator when sampling
            batches. If None (default value), a new default generator will be used.
        max_trajectory_length (int, optional): if given, indicates that trajectory
    ...
    def __init__(
        ...
        obs_shape: Sequence[int],
        action_shape: Sequence[int],
        obs_type: Type = np.float32, # the lowest level of obs is still np.float32
        action_type: Type = np.float32,
        reward_type: Type = np.float32,
        rng: Optional[np.random.Generator] = None,
        max_trajectory_length: Optional[int] = None,
        ...
        if max_trajectory_length:
            ...
            capacity += max_trajectory_length
        # TODO replace all of these with a transition batch
        # TODO: replace these with c and delta, each state is a tuple
        self.obs = Box(np.empty((capacity, *obs_shape), dtype=obs_type), np.empty((capacity, *obs_shape), dtype=obs_type))
        self.next_obs = Box(np.empty((capacity, *obs_shape), dtype=obs_type), np.empty((capacity, *obs_shape), dtype=obs_type))
        self.action = Box(np.empty((capacity, *action_shape), dtype=action_type), np.empty((capacity, *action_shape), dtype=action_type))
        self.reward = np.empty(capacity, dtype=reward_type)
        self.done = np.empty(capacity, dtype=bool) # TODO: if all the states in the set is done, then done; otherwise, not done
        if rng is None:
            self._rng = np.random.default_rng()
        else:
            self._rng = rng
    ...
    @staticmethod
    # todo: should I change this part?
    def _check_overlap(segment1: Tuple[int, int], segment2: Tuple[int, int]) -> bool:
        s1, e1 = segment1
        s2, e2 = segment2
        return (s1 <= s2 < e1) or (s1 < e2 <= e1)
    ...
    def add(
        self,
        obs, # Box(np.ndarray)
        action, # Box(np.ndarray)
        next_obs, # Box(np.ndarray)
        reward, # number 
        done: bool,
    ):
        """Adds a symbolic transition (s#, a#, s'#, r, done) to the replay buffer.

        Args:
            obs (box(np.ndarray)): the observation at time t.
            action (Box(np.ndarray)): the action at time t.
            next_obs (Box(np.ndarray)): the observation at time t + 1.
            reward (float): the reward at time t + 1.
            done (bool): a boolean indicating whether the episode ended or not.
        """
        # self.obs: tuple of np.ndarray; 
        self.obs.c[self.cur_idx], self.obs.delta[self.cur_idx] = obs.c, obs.delta
        self.next_obs.c[self.cur_idx], self.next_obs.delta[self.cur_idx] = next_obs.c, next_obs.delta
        self.action.c[self.cur_idx], self.action.delta[self.cur_idx] = action.c, action.delta
        self.reward[self.cur_idx] = reward
        self.done[self.cur_idx] = done
    ...
    def add_batch(
        ...
        def copy_from_to(buffer_start, batch_start, how_many):
            buffer_slice = slice(buffer_start, buffer_start + how_many)
            batch_slice = slice(batch_start, batch_start + how_many)
            np.copyto(self.obs.c[buffer_slice], obs.c[batch_slice])
            np.copyto(self.obs.delta[buffer_slice], obs.delta[batch_slice])
            np.copyto(self.action.c[buffer_slice], action.c[batch_slice])
            np.copyto(self.action.delta[buffer_slice], action.delta[batch_slice])
            np.copyto(self.reward[buffer_slice], reward[batch_slice])
            np.copyto(self.next_obs.c[buffer_slice], next_obs.c[batch_slice])
            np.copyto(self.next_obs.delta[buffer_slice], next_obs.delta[batch_slice])
            np.copyto(self.done[buffer_slice], done[batch_slice])
        _batch_start = 0
        buffer_end = self.cur_idx + len(obs)
        if buffer_end > self.capacity:
    ...
    def _batch_from_indices(self, indices: Sized) -> TransitionBatch:
        obs = self.obs[indices] # a new box
        next_obs = self.next_obs[indices]
        action = self.action[indices]
        reward = self.reward[indices]
        done = self.done[indices]
    ...
    def save(self, save_dir: Union[pathlib.Path, str]):
        # Save the symbolic trajectories. Separate the c and delta
        """Saves the data in the replay buffer to a given directory.
        ...
        Args:
            save_dir (str): the directory to save the data to. File name will be
                replay_buffer.npz.
        """
        path = pathlib.Path(save_dir) / "symbolic_replay_buffer.npz"
        np.savez(
            path,
            obs_c=self.obs.c[: self.num_stored],
            obs_delta=self.obs.delta[: self.num_stored],
            next_obs_c=self.next_obs.c[: self.num_stored],
            next_obs_delta=self.next_obs.delta[: self.num_stored],
            action_c=self.action.c[: self.num_stored],
            action_delta=self.action.delta[: self.num_stored],
            reward=self.reward[: self.num_stored],
            done=self.done[: self.num_stored],
            trajectory_indices=self.trajectory_indices or [],
        )
    ...
    def load(self, load_dir: Union[pathlib.Path, str]):
        ...
        Args:
            load_dir (str): the directory where the buffer is stored.
        """
        path = pathlib.Path(load_dir) / "symbolic_replay_buffer.npz"
        data = np.load(path)
        num_stored = len(data["obs_c"])
        self.obs.c[:num_stored] = data["obs_c"]
        self.obs.delta[:num_stored] = data["obs_delta"]
        self.next_obs.c[:num_stored] = data["next_obs_c"]
        self.next_obs.delta[:num_stored] = data["next_obs_delta"]
        self.action.c[:num_stored] = data["action_c"]
        self.action.delta[:num_stored] = data["action_delta"]
        self.reward[:num_stored] = data["reward"]
        self.done[:num_stored] = data["done"]
        self.num_stored = num_stored
        self.cur_idx = self.num_stored % self.capacity
