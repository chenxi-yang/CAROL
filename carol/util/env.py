...
) -> Tuple[carol.util.types.TermFnType, Optional[carol.util.types.RewardFnType]]:
    import env

    term_fn = getattr(env.termination_fns, cfg.overrides.term_fn)
    if hasattr(cfg.overrides, "reward_fn") and cfg.overrides.reward_fn is not None:
        reward_fn = getattr(env.reward_fns, cfg.overrides.reward_fn)
    else:
        reward_fn = getattr(env.reward_fns, cfg.overrides.term_fn, None)
...
def _handle_learned_rewards_and_seed(
    cfg: Union[omegaconf.ListConfig, omegaconf.DictConfig],
    env: gym.Env,
    reward_fn: carol.util.types.RewardFnType,
) -> Tuple[gym.Env, carol.util.types.RewardFnType]:
...
def _legacy_make_env(
    cfg: Union[omegaconf.ListConfig, omegaconf.DictConfig],
) -> Tuple[gym.Env, carol.util.types.TermFnType, Optional[carol.util.types.RewardFnType]]:
    if "gym___" in cfg.overrides.env:
        env = gym.make(cfg.overrides.env.split("___")[1])
        term_fn, reward_fn = _get_term_and_reward_fn(cfg)
    else:
        
        if cfg.overrides.env == "halfcheetah":
            env = gym.make("HalfCheetah-v2")
            term_fn = carol.env.termination_fns.halfcheetah
            reward_fn = carol.env.reward_fns.halfcheetah
        elif cfg.overrides.env == "inverted_pendulum":
            env = gym.make("InvertedPendulum-v2")
            # env = carol.env.inverted_pendulum.InvertedPendulumEnv()
            term_fn = carol.env.termination_fns.inverted_pendulum
            reward_fn = carol.env.reward_fns.inverted_pendulum
        elif cfg.overrides.env == "hopper":
            env = gym.make("Hopper-v2")
            term_fn = carol.env.termination_fns.hopper
            reward_fn = carol.env.reward_fns.hopper
        elif cfg.overrides.env == "walker2d":
            env = gym.make("Walker2d-v2")
            term_fn = carol.env.termination_fns.walker2d
            reward_fn = carol.env.reward_fns.walker2d
        elif cfg.overrides.env == "ant":
            # env = carol.env.swimmer.SwimmerEnv()
            env = gym.make("Ant-v2")
            term_fn = carol.env.termination_fns.ant
            reward_fn = carol.env.reward_fns.ant
        elif cfg.overrides.env == "humanoid":
            env = gym.make("Humanoid-v2")
            term_fn = carol.env.termination_fns.humanoid
            reward_fn = carol.env.reward_fns.humanoid
        else:
...
class EnvHandler(ABC):
    ...
    def rollout_env(
        ...
        Args:
            ...
            agent (:class:`carol.planning.Agent`, optional): if given, an agent to obtain actions.
