...
if torch.cuda.is_available() and not 'NOCUDA' in os.environ:
    ones = lambda *args, **cargs: torch.ones(*args, **cargs).cuda()
    zeros = lambda *args, **cargs: torch.zeros(*args, **cargs).cuda()
    eye = lambda *args, **cargs: torch.eye(*args, **cargs).cuda()
    # use_cuda = True
else:
    ones = torch.ones
    zeros = torch.zeros
...
def add_disturbance(data, disturbance):
    # add random disturbance
    shape = data.shape
    noise = np.random.uniform(low=-1.0, high=1.0, size=shape)
    disturbed_data = data + noise * disturbance
...
def create_one_dim_tr_model(
    ...
    model_normalization: Optional[bool] = None,
):
    ...
    :class:`carol.models.OneDTransitionRewardModel` (see its documentation for explanation of some
    ...
    The configuration should be structured as follows::
        ...
        -cfg
          -dynamics_model
            -model
              -_target_ (str): model Python class # should use a normal model strucutre
    ...
    Returns:
        (:class:`carol.models.OneDTransitionRewardModel`): the model created.
    ...
    if model_cfg._target_ == "carol.models.BasicEnsemble":
    ...
    if name_obs_process_fn:
        ...
        obs_process_fn = None
    # dynamics_model.model is the model
    dynamics_model = carol.models.OneDTransitionRewardModel(
        model,
...
def create_replay_buffer(
    ...
    Returns:
        (:class:`carol.replay_buffer.ReplayBuffer`): the replay buffer.
    """
    dataset_size = (
        cfg.algorithm.get("dataset_size", None) if "algorithm" in cfg else None
    )
    # print(f"dataset_size: {dataset_size}")
    if not dataset_size or dataset_size == "None":
        dataset_size = cfg.overrides.num_steps
    # print(f"dataset_size: update: {dataset_size}")
    # exit(0)
    ...
    if symbolic: # the states, actions are in the type of Box<np.array, np.array>
        replay_buffer = SymbolicReplayBuffer(
            dataset_size,
            obs_shape,
            act_shape,
            obs_type=obs_type,
...
def get_basic_buffer_iterators(
    ...
    Args:
        replay_buffer (:class:`carol.util.ReplayBuffer`): the replay buffer from which
    ...
    Returns:
        (tuple of :class:`carol.replay_buffer.TransitionIterator`): the training
...
def get_sequence_buffer_iterator(
    ...
    Args:
        ...
        use_simple_sampler (int): if ``True``, returns an iterator of type
            :class:`carol.replay_buffer.SequenceTransitionSampler` instead of
            :class:`carol.replay_buffer.SequenceTransitionIterator`.
...
def train_model_and_save_model_and_data(
    model: carol.models.Model, # the environment model
    model_trainer: carol.models.ModelTrainer,
    cfg: omegaconf.DictConfig,
    replay_buffer: ReplayBuffer,
    ...
    Args:
        model (:class:`carol.models.Model`): the model to train.
        model_trainer (:class:`carol.models.ModelTrainer`): the model trainer.
        ...
        replay_buffer (:class:`carol.util.ReplayBuffer`): the replay buffer to use.
        ...
        callback (callable, optional): if provided, this function will be called after
            every training epoch. See :class:`carol.models.ModelTrainer` for signature.
    """
    # print(f"---------------get_basic_buffer_iterators-------------")
    dataset_train, dataset_val = carol.util.common.get_basic_buffer_iterators(
        replay_buffer,
    ...
    # print(f"model training: {training_losses, val_scores}")
...
def rollout_model_env(
    ...
    agent: Optional[carol.planning.Agent] = None,
    num_samples: int = 1,
    ...
    Args:
         model_env (:class:`carol.models.ModelEnv`): the dynamics model environment to simulate.
         initial_obs (np.ndarray): initial observation to start the episodes.
         plan (np.ndarray, optional): sequence of actions to execute.
         agent (:class:`carol.planning.Agent`): an agent to generate a plan before
...
def rollout_agent_trajectories(
    ...
    Args:
        ...
        agent (:class:`carol.planning.Agent`): the agent used to generate an action.
        ...
        replay_buffer (:class:`carol.util.ReplayBuffer`, optional):
        ...
        agent_uses_low_dim_obs (bool): only valid if env is of type
            :class:`carol.env.MujocoGymPixelWrapper` and replay_buffer is not ``None``.
    ...
    while True:
        obs = env.reset()
        if disturbance: # add disturbance
            obs = add_disturbance(obs, disturbance)
        agent.reset()
        done = False
        total_reward = 0.0
        while not done:
            ...
            obs = next_obs
            if disturbance: # add disturbance for the next starting state
                obs = add_disturbance(obs, disturbance)
            total_reward += reward
            step += 1
...
def step_env_and_add_to_buffer(
    ...
    Args:
        ...
        agent (:class:`carol.planning.Agent`): the agent used to generate an action.
        agent_kwargs (dict): any keyword arguments to pass to `agent.act()` method.
        replay_buffer (:class:`carol.util.ReplayBuffer`): the replay buffer
            containing stored data.
        callback (callable, optional): a function that will be called using the generated
            transition data `(obs, action. next_obs, reward, done)`.
        agent_uses_low_dim_obs (bool): only valid if env is of type
            :class:`carol.env.MujocoGymPixelWrapper`. If ``True``, instead of passing the obs
            produced by env.reset/step to the agent, it will pass
            obs = env.get_last_low_dim_obs(). This is useful for rolling out an agent
            trained with low dimensional obs, but collect pixel obs in the replay buffer.
    ...
    if agent_uses_low_dim_obs and not hasattr(env, "get_last_low_dim_obs"):
        raise RuntimeError(
            ...
            "env of type carol.env.MujocoGymPixelWrapper."
        )
    if agent_uses_low_dim_obs:
...
def extract_component(x_l, x_r, single_dim_component, action_space, action_ini_range):
    # TODO: add a recursive splitting algorithm
    # debug 
    assert(len(x_l) == len(x_r))
    component_length_list = list()
    for i in range(len(x_l)):
        component_length = (x_r[i] - x_l[i]) / single_dim_component
        component_length_list.append(component_length)
    center_list, width_list = list(), list()
    # action_c, action_width = (action_ini_range[1] + action_ini_range[0]) / 2.0, (action_ini_range[1] - action_ini_range[0]) / 2.0

    if len(x_l) == 4:
        for i in range(single_dim_component):
            l0 = x_l[0] + i * component_length_list[0]
            r0 = x_l[0] + (i + 1) * component_length_list[0]
            center_0, width_0 = (r0 + l0) / 2.0, (r0 - l0) / 2.0
            for j in range(single_dim_component):
                l1 = x_l[1] + j * component_length_list[1]
                r1 = x_l[1] + (j + 1) * component_length_list[1]
                center_1, width_1 = (r1 + l1) / 2.0, (r1 - l1) / 2.0
                for m in range(single_dim_component):
                    l2 = x_l[2] + m * component_length_list[2]
                    r2 = x_l[2] + (m + 1) * component_length_list[2]
                    center_2, width_2 = (r2 + l2) / 2.0, (r2 - l2) / 2.0
                    for n in range(single_dim_component):
                        l3 = x_l[3] + n * component_length_list[3]
                        r3 = x_l[3] + (n + 1) * component_length_list[3]
                        center_3, width_3 = (r3 + l3) / 2.0, (r3 - l3) / 2.0
                        center_list.append([center_0, center_1, center_2, center_3])
                        width_list.append([width_0, width_1, width_2, width_3])
    elif len(x_l) == 2:
        for i in range(single_dim_component):
            l0 = x_l[0] + i * component_length_list[0]
            r0 = x_l[0] + (i + 1) * component_length_list[0]
            center_0, width_0 = (r0 + l0) / 2.0, (r0 - l0) / 2.0
            for j in range(single_dim_component):
                if x_l[1] == x_r[1]: # if the initial sets is a point, no splitting
                    center_list.append([center_0, x_l[1]])
                    width_list.append([width_0, 0.0])
                    break 
                l1 = x_l[1] + j * component_length_list[1]
                r1 = x_l[1] + (j + 1) * component_length_list[1]
                center_1, width_1 = (r1 + l1) / 2.0, (r1 - l1) / 2.0
                center_list.append([center_0, center_1])
                width_list.append([width_0, width_1])
    else:
        raise NotImplementedError(f"Error: range of input states not covered.")
...
def in_interval(x_l, x_r, y, print=False):
    # check if x in y
    # add a rounding operation
    if x_l >= y.left - var(1e-6) and x_r <= y.right + var(1e-6):
        return True
    else:
        return False


def build_env_dataset(env, cfg, portion=0.3, total_steps=200000):
    trial_length = cfg.overrides.trial_length
    act_list, obs_list = list(), list()
    train_size, test_size = int(total_steps * (1-portion)), int(total_steps * portion)
    step = 0
    for i in range(total_steps):
        obs = env.reset()
        done = False
        # assume random action
        current_step = 0
        while not done:
            last_state = obs
            action = env.random_action() # assume random actions
            obs, *_ = env.real_step(action)
            act_list.append(np.concatenate((last_state, action)))
            obs_list.append(obs)
            step += 1
            current_step += 1
            if current_step >= trial_length: # do not exceed the maximum length
                break
            if step >= total_steps:
                break
        if step >= total_steps:
    ...
    c = list(zip(act_list, obs_list))
    random.shuffle(c)
    actions, obs = zip(*c)
    train_actions, test_actions = actions[:train_size], actions[train_size+1:]
    train_obs, test_obs = obs[:train_size], obs[train_size+1:]
    train_actions, train_obs = np.array(train_actions), np.array(train_obs)
    test_actions, test_obs = np.array(test_actions), np.array(test_obs)

    dtrain = (train_actions, train_obs)
    dtest = (test_actions, test_obs)
    return dtrain, dtest


def data_loader(d, bs):
    actions, obs = d
    assert(len(actions)==len(obs))
    k = int((len(actions) - 1) / bs)
    # print(k)
    for i in range(k+1):
        X, y = actions[bs*i:bs*(i+1)], obs[bs*i:bs*(i+1)]
...
def check_interval_in(x, target):
    if x >= target[0] and x <= target[1]:
        return True
    else:
        return False

def get_last_logs(path):
    log_dict = dict()
    f = open(path, 'r')
    for line in f:
        pass
    ...
    datas = re.split(f"; ", last_line)
    for data in datas:
        name = data.split(f": ")[0]
        if name in ['Step']: # , 'Abstraction Ratio']:
            number = int(data.split(f": ")[1])
        else:
            # print(data)
            if data.split(f": ")[1] == 'None':
                number = None
            else:
                number = float(data.split(f": ")[1])
        log_dict[name] = number
    return log_dict
