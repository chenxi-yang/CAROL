...
class Normalizer:
    ...
    def update_stats(self, data: carol.util.types.TensorType):
    ...
    def normalize(self, val: Union[float, carol.util.types.TensorType]) -> torch.Tensor:
        ...
        elif isinstance(val, carol.domain.box.Box): # val is the box
            if isinstance(val.c, np.ndarray):
            # normalize
                val = val.torch_from_numpy().to(self.device)
            return val.sub_l(self.mean).mul(1/self.std)
        return (val - self.mean) / self.std

    def denormalize(self, val: Union[float, carol.util.types.TensorType]) -> torch.Tensor:
